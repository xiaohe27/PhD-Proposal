...
let get_predicates f =
  let rec get_preds preds = function     
    ...
    | Pred p ->  p :: preds
    | Card f
    | Neg f -> get_preds preds f
    | Exists (v,f) 
    | ForAll (v,f) -> get_preds preds f
    | Prev (intv,f) 
    | Next (intv,f) 
    | Eventually (intv,f) 
    | Once (intv,f) 
    | Always (intv,f) 
    | PastAlways (intv,f) -> get_preds preds f
    | And (f1,f2)
    | Or (f1,f2)
    | Implies (f1,f2)
    | Equiv (f1,f2) -> (get_preds preds f1) @ (get_preds preds f2)
    | Since (intv,f1,f2)
    | Until (intv,f1,f2) -> (get_preds preds f1) @ (get_preds preds f2)
  in
  get_preds [] f
...
let get_tuple_filter f =
  ...
  let rec get_tuples tuples = function (* formula *)
    ...
    | Card f
    | Neg f -> get_tuples tuples f
    | Exists (v,f) 
    | ForAll (v,f) -> get_tuples tuples f
    | Prev (intv,f) 
    | Next (intv,f) 
    | Eventually (intv,f) 
    | Once (intv,f) 
    | Always (intv,f) 
    | PastAlways (intv,f) -> get_tuples tuples f
    | And (f1,f2)
    | Or (f1,f2)
    | Implies (f1,f2)
    | Equiv (f1,f2) -> get_tuples (get_tuples tuples f1) f2
    | Since (intv,f1,f2)
    | Until (intv,f1,f2) -> get_tuples (get_tuples tuples f1) f2
  in
